...
async function getMarkets(limit=100, offset=0) {
  const data = await safeFetch(`${GAMMA_API}/markets?limit=${limit}&offset=${offset}&active=true&closed=false&order=volume&ascending=false`);
  return data || [];
}

async function getMarketById(id) {
  return await safeFetch(`${GAMMA_API}/markets/${id}`);
}
...
async function runScan() {
  ...
  // Filter to viable markets
  const viable = allMarkets.filter(m => {
    ...
    const hours   = (endDate-new Date())/3600000;
